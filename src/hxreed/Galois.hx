 ...
 */
class Galois {
    public static var FIELD_SIZE:Int = 256;
     ...
     */
     public static var GENERATING_POLYNOMIAL:Int = 29;

    public static var LOG_TABLE = [
    ...
    public static var EXP_TABLE = [
        1,    2,   4,   8,  16,  32, 64, 128, 
        29,  58, 116, 232, 205, 135, 19, 38, 
        76, 152, 45, 90, 180, 117, 234, 201, 
        143, 3, 6, 12, 24, 48, 96, 192, 
        157, 39, 78, 156, 37, 74, 148, 53, 
        106, 212, 181, 119, 238, 193, 159, 
        35, 70, 140, 5, 10, 20, 40, 80, 
        160, 93, 186, 105, 210, 185, 111, 
        222, 161, 95, 190, 97, 194, 153, 47, 
        94, 188, 101, 202, 137, 15, 30, 60, 
        120, 240, 253, 231, 211, 187, 107, 214, 
        177, 127, 254, 225, 223, 163, 91, 182, 
        113, 226, 217, 175, 67, 134, 17, 34, 68, 
        136, 13, 26, 52, 104, 208, 189, 103, 206, 
        ...
        230, 209, 191, 99, 198, 145, 63, 126, 252, 229,
         ...
         75, 150, 49, 98, 196, 149, 55, 110, 220, 165, 
         87, 174, 65, 130, 25, 50, 100, 200, 141, 7, 14, 
         28, 56, 112, 224, 221, 167, 83, 166, 81, 162, 89, 
         178, 121, 242, 249, 239, 195, 155, 43, 86, 172, 69, 
         138, 9, 18, 36, 72, 144, 61, 122, 244, 245, 247, 243, 
         251, 235, 203, 139, 11, 22, 44, 88, 176, 125, 250, 233, 
     ...
     */
     public static var MULTIPLICATION_TABLE:Array<Array<Int>> = generateMultiplicationTable();
     ...
     * you should inline this function: it's just XOR.
     */
    public static function subtract(a: Int, b: Int) {
        return (a ^ b);
    }
     ...
     */
     public static function multiply(a: Int, b: Int): Int {
        if (a == 0 || b == 0) {
            ...
            var logA = LOG_TABLE[a & 0xFF];
            var logB = LOG_TABLE[b & 0xFF];
     ...
     */
     public static function divide(a: Int, b: Int): Int {
        ...
        if (b == 0) {
            throw new haxe.exceptions.ArgumentException("b", "Divisor is 0");
        }
        var logA = LOG_TABLE[a & 0xFF];
        var logB = LOG_TABLE[b & 0xFF];
        var logResult = logA - logB;
     ...
     */
     public static function exp(a: Int, n: Int): Int {
        if (n == 0) {
            ...
            var logA: Int = LOG_TABLE[a & 0xFF];
            var logResult: Int = logA * n;
            while (255 <= logResult) {
                logResult -= 255;
            }
            return EXP_TABLE[logResult];
     ...
     */
     public static function generateLogTable(polynomial: Int): Array<Int> {
        ...
        for (log in 0...(FIELD_SIZE - 1)) {
            if (result[b] != -1) {
                throw new haxe.Exception("BUG: duplicate logarithm (bad polynomial?)");                
            }
            result[b] = log;
            b = (b << 1);
            if (FIELD_SIZE <= b) {
     ...
     */
     public static function generateExpTable(logTable: Array<Int>): Array<Int> {
        var result = new Array<Int>();
        result.resize(FIELD_SIZE * 2 - 2);

        for (i in 1...FIELD_SIZE) {
            var log: Int = logTable[i];
            result[log] = i;
     ...
     */
     ...
     public static function generateMultiplicationTable(): Array<Array<Int>> {
        var result = new Array<Array<Int>>();
        result.resize(FIELD_SIZE);
        for (i in 0...result.length) {
            result[i] = new Array<Int>();
            result[i].resize(FIELD_SIZE);
        }
        for (a in 0...256)
            for (b in 0...256)
     ...
     */
     public static function allPossiblePolynomials(): Array<Int> {
        var result = new Array<Int>();
        for (i in 0...FIELD_SIZE) {
            try {
                generateLogTable(i);
